...
import { Send, Bot, User, Sparkles, Paperclip, Mic, ChevronLeft, Plus, MessageSquare, MoreHorizontal, BarChart3 } from 'lucide-react';
...
const Message = ({ role, content, reasoning, chartData }) => {
  const renderChart = () => {
        ...
        {chartData && (
          <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-slate-800">{chartData.title || 'Generated Chart'}</h3>
            </div>
            <div className="h-64">
              {renderChart()}
            </div>
            {chartData.description && (
              <p className="text-sm text-slate-600 mt-2">{chartData.description}</p>
            )}
          </div>
        )}
...
const Assistant = () => {
  ...
  const handleSend = async (text = input) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/assistant/chat`, { message: text });
      ...
      setMessages(prev => [...prev, {
        ...
        chartData: aiResponse?.chart_data
      }]);
    } catch (error) {
